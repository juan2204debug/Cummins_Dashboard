...
function dateFromExcel(v){
  ...
  if(typeof v==="number"&&isFinite(v)){
    // Excel serial date: 1 = 1900-01-01; JS epoch conversion adjusted by 25569 days
...
async function parseXLSX(file){
  ...
  const sfn = sucFromName(file.name);
  const pfn = periodFromName(file.name) || {y:new Date().getFullYear(),mo:new Date().getMonth()+1};
  ...
  rows.forEach(r=>{
    ...
    // 2. Try Fecha factura
    const fv=r[c.fecha];
    const dFactura=dateFromExcel(fv);
    if(dFactura){mo=dFactura.getMonth()+1;yr=dFactura.getFullYear();}
    // 3. Fallback to Fecha de contabilización
    else if(cFechaContab&&r[cFechaContab]){
      const dContab=dateFromExcel(r[cFechaContab]);
      if(dContab){mo=dContab.getMonth()+1;yr=dContab.getFullYear();}
    }
  ...
  const ingest=useCallback(async fs=>{
    ...
    for(const f of fs){
      ...
      try{
        ...
        console.error("Error leyendo Excel",f.name,e);
        errors.push(`${f.name}: ${e?.message||e}`);
      }
    }
    if(!newRecs.length){
      setLoading(false);
      if(errors.length)alert(`No se pudo cargar ningún Excel:\n${errors.join("\n")}`);
      return;
    }
    const labelsLoaded = new Set(newFiles.map(f=>f.label));
    setAllRecs(prev=>{
      // Remove seed placeholder and replace same-label files when re-uploaded
      const base = prev.filter(r=>r.file!=="2026_04_K27"&&!labelsLoaded.has(r.file));
      return [...base,...newRecs];
    });
    setFiles(prev=>{
      const base = prev.filter(f=>f.label!=="2026_04_K27"&&!labelsLoaded.has(f.label));
