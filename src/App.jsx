...
import { AreaChart, Area, BarChart, Bar, LineChart, Line, XAxis, YAxis,
  ...
  Cell, ScatterChart, Scatter, ZAxis } from "recharts";

const B = {
  red:"#CC0000",redDk:"#990000",redBg:"#150000",redDim:"#220000",
  white:"#FFFFFF",g1:"#E0E0E0",g2:"#A0A0A0",g3:"#606060",
  g4:"#333333",bg:"#0A0A0A",surf:"#111111",panel:"#161616",
  card:"#1A1A1A",border:"#2A2A2A",borderHi:"#3A3A3A",
  green:"#16A34A",greenDim:"#071A07",greenLt:"#4ADE80",
  amber:"#D97706",amberDim:"#1A0E00",amberLt:"#FCD34D",
  blue:"#2563EB",blueDim:"#050F20",blueLt:"#93C5FD",
  teal:"#0D9488",
  ch:["#CC0000","#E57373","#D97706","#16A34A","#2563EB","#7C3AED","#DB2777","#0D9488","#CA8A04","#EA580C","#0891B2","#65A30D"],
};

const Logo=()=>(<svg width={38} height={32} viewBox="0 0 96 80"><polygon points="48,2 90,22 90,58 48,78 6,58 6,22" fill={B.red}/><text x="48" y="44" textAnchor="middle" fill="#fff" fontFamily="Arial Black,sans-serif" fontSize="16" fontWeight="900" letterSpacing=".5">CUMMINS</text><text x="48" y="58" textAnchor="middle" fill="rgba(255,255,255,.6)" fontFamily="Arial,sans-serif" fontSize="7" letterSpacing="2">ZONA SUR</text></svg>);
...
function parseAndAggregate(fileLabel,rows){
  ...
  rows.forEach(r=>{
    ...
    if(!byClient[cli])byClient[cli]={id:cli,nm:clinm,sector:sec,rep,ing:0,cst:0,mgn:0,vol:0,lineas:0,months:new Set(),fyl};
  ...
  return{
    ...
    cols:Object.fromEntries(Object.entries(cols).map(([k,v])=>[k,v||null])),headers:headers.slice(0,25),
  };
}
...
function autoAnalysis(clients,familia,reps,kpi,repNames){
  ...
  familia.filter(f=>f.mgnP<0&&Math.abs(f.ing)>200).forEach(f=>alerts.push({icon:"🔴",t:`${f.f2}: margen ${f.mgnP.toFixed(0)}%`,b:`${f$(Math.abs(f.mgn))} de pérdida. Actualizar precios SAP.`}));
  const top2pct=clients.slice(0,2).reduce((s,c)=>s+(c.pct||0),0);
  if(top2pct>50)alerts.push({icon:"🟡",t:`Concentración: ${top2pct.toFixed(0)}% en 2 clientes`,b:"Riesgo de dependencia. Diversificar."});
  clients.filter(c=>c.mgnP<-1&&c.ing>300).slice(0,2).forEach(c=>alerts.push({icon:"🟠",t:`${c.nm.split(" ").slice(0,3).join(" ")}: margen negativo`,b:`${f$(c.ing)} con ${f$(c.mgn)} pérdida.`}));
  familia.filter(f=>f.mgnP>30&&f.ing>1000).slice(0,3).forEach(f=>opps.push({icon:"💎",t:`${f.f2}: ${f.mgnP.toFixed(0)}% margen`,b:"Escalar ventas de esta línea."}));
  clients.filter(c=>c.mgnP>35&&c.ing>500&&c.meses<3).slice(0,2).forEach(c=>opps.push({icon:"🚀",t:`${c.nm.split(" ").slice(0,3).join(" ")}: potencial sin desarrollar`,b:`${c.mgnP.toFixed(0)}% margen en ${c.meses} mes(es).`}));
  if(alerts.some(a=>a.icon==="🔴"))actions.push(`Revisión urgente: ${familia.filter(f=>f.mgnP<0).map(f=>f.f2).slice(0,2).join(", ")}`);
  if(clients[0])actions.push(`Visita a ${clients[0].nm.split(" ").slice(0,2).join(" ")} — ${clients[0].pct?.toFixed(0)||"?"}% del ingreso`);
  actions.push("Prospectar nuevos clientes para reducir concentración");
  return{alerts,opps,actions:actions.slice(0,5)};
}
...
const TT=({active,payload,label})=>{
  ...
  return(<div style={{background:B.g4,border:`1px solid ${B.borderHi}`,borderRadius:8,padding:"9px 13px",fontSize:11,boxShadow:"0 8px 24px #0009"}}>
    ...
    {payload.map((p,i)=>(<p key={i} style={{color:p.color||B.white,margin:"2px 0",display:"flex",gap:16,justifyContent:"space-between"}}><span style={{color:B.g2,fontSize:10}}>{p.name}</span><strong>{Math.abs(p.value??0)>50?f$(p.value):typeof p.value==="number"?p.value.toFixed(1):p.value}</strong></p>))}
  </div>);
};
...
export default function App(){
  ...
  const [fSuc,setFSuc]=useState([]);const [fFY,setFY]=useState([]);const [fQ,setFQ]=useState([]);
  const [fSec,setFSec]=useState([]);const [fFam,setFFam]=useState([]);const [fRep,setFRep]=useState([]);
  const [fSeg,setFSeg]=useState([]);const [search,setSearch]=useState("");const [selCli,setSelCli]=useState(null);
  ...
  const [pRows,setPRows]=useState("sector");const [pCols,setPCols]=useState("fyl");
  const [pVal,setPVal]=useState("ing");const [pSort,setPSort]=useState("desc");
  ...
  const ingest=useCallback(async(files)=>{
    ...
    // Process all files, then single state update
    const results=[];
    for(const f of xlsFiles){
      try{
        const agg=await loadXLSX(f);
  ...
  const kpi=useMemo(()=>{
    ...
    const mgn=ing-cst;
    return{ing,cst,mgn,mgnP:ing?mgn/ing*100:0,vol:filteredAggs.reduce((s,f)=>s+f.totalVol,0),lineas:filteredAggs.reduce((s,f)=>s+f.salesRows,0),clis:new Set(filteredAggs.flatMap(f=>f.clients.map(c=>c.id))).size};
  },[filteredAggs]);
  ...
  const allClients=useMemo(()=>{
    ...
    filteredAggs.forEach(f=>f.clients.forEach(c=>{
      ...
      const x=m[c.id];x.ing+=c.ing;x.cst+=c.cst;x.mgn+=c.mgn;x.vol+=c.vol;x.lineas+=c.lineas;x.meses=Math.max(x.meses,c.meses);
      if(c.sector&&c.sector!=="Otros")x.sector=c.sector;if(c.rep&&c.rep!=="0")x.rep=c.rep;
    }));
  ...
  const allSector=useMemo(()=>{
    const m={};
    filteredAggs.forEach(f=>f.sector.forEach(s=>{if(!m[s.sec])m[s.sec]={sec:s.sec,ing:0,lineas:0,clientes:0};m[s.sec].ing+=s.ing;m[s.sec].lineas+=s.lineas;m[s.sec].clientes+=s.clientes;}));
  ...
  const allArticles=useMemo(()=>{
    const m={};
    filteredAggs.forEach(f=>f.articles.forEach(a=>{if(!m[a.art])m[a.art]={art:a.art,f2:a.f2,f1:a.f1,ing:0,cst:0,mgn:0,vol:0};m[a.art].ing+=a.ing;m[a.art].cst+=a.cst;m[a.art].mgn+=a.mgn;m[a.art].vol+=a.vol;}));
  ...
  const PDIMS={sector:"Sector",fyl:"Año Fiscal",f2:"Sub-familia",f1:"Familia",rep:"Ejecutivo",suc:"Sucursal"};
  ...
  const analysis=useMemo(()=>autoAnalysis(allClients,allFamilia,allReps,kpi,repNames),[allClients,allFamilia,allReps,kpi,repNames]);
  ...
  const Pill=({label,active,color=B.red,onClick})=>(<button onClick={onClick} style={{background:active?`${color}22`:"transparent",border:`1px solid ${active?color:B.border}`,color:active?color:B.g3,borderRadius:20,padding:"3px 10px",fontSize:10,cursor:"pointer",fontFamily:"inherit",fontWeight:active?700:400,letterSpacing:".04em",transition:"all .12s",whiteSpace:"nowrap",lineHeight:"16px"}}>{label}</button>);
  const KCard=({icon,label,value,sub,color=B.red,warn=false})=>(<div style={{background:warn?B.redBg:B.card,border:`1px solid ${warn?B.red:B.border}`,borderRadius:10,padding:"14px 16px",borderTop:`2px solid ${warn?B.red:color}`,flex:"1 1 150px",minWidth:148,maxWidth:260}}><div style={{fontSize:9,color:B.g3,letterSpacing:".12em",textTransform:"uppercase",marginBottom:5,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{icon} {label}</div><div style={{fontSize:17,fontWeight:800,color:warn?B.red:color,lineHeight:1.2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{value}</div>{sub&&<div style={{fontSize:10,color:B.g3,marginTop:4,lineHeight:1.4}}>{sub}</div>}</div>);
  ...
  const SBadge=({s})=>{const c=s==="TOP"?B.amberLt:s==="MED"?B.blueLt:B.g3;const bg=s==="TOP"?B.amberDim:s==="MED"?B.blueDim:B.g4;return <span style={{background:bg,color:c,borderRadius:3,padding:"2px 6px",fontSize:9,fontWeight:700}}>{s}</span>;};

  const Sidebar=()=>(<div style={{display:"flex",flexDirection:"column",gap:14}}>
    <div><div style={{fontSize:9,color:B.g3,letterSpacing:".14em",fontWeight:700,marginBottom:5}}>BÚSQUEDA</div><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Cliente, código..." style={{width:"100%",background:B.panel,border:`1px solid ${B.border}`,borderRadius:5,color:B.white,fontFamily:"inherit",fontSize:11,padding:"6px 9px",outline:"none"}}/></div>
    {[{lbl:"SUCURSAL",opts:opts.sucs,active:fSuc,set:setFSuc,color:B.red},{lbl:"AÑO FISCAL",opts:opts.fys,active:fFY,set:setFY,color:B.blue},{lbl:"TRIMESTRE",opts:["1","2","3","4"],labels:["Q1 Abr","Q2 Jul","Q3 Oct","Q4 Ene"],active:fQ,set:setFQ,color:B.amber},{lbl:"SECTOR",opts:opts.secs,active:fSec,set:setFSec,color:B.teal,maxH:80},{lbl:"FAMILIA",opts:opts.fams,active:fFam,set:setFFam,color:B.red,maxH:80},{lbl:"EJECUTIVO",opts:opts.reps,active:fRep,set:setFRep,color:B.amber,nm:repNames},{lbl:"SEGMENTO",opts:["TOP","MED","LOW"],active:fSeg,set:setFSeg,color:B.amber}].map(({lbl,opts:o,labels,active,set,color,nm,maxH})=>(
      <div key={lbl}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}><span style={{fontSize:9,color:B.g3,letterSpacing:".1em",fontWeight:700}}>{lbl}</span>{active.length>0&&<button onClick={()=>set([])} style={{background:"none",border:"none",color:B.red,cursor:"pointer",fontSize:9,fontFamily:"inherit",padding:0}}>✕</button>}</div>
      <div style={{display:"flex",flexWrap:"wrap",gap:3,maxHeight:maxH||9999,overflowY:maxH?"auto":"visible"}}>{(o||[]).map((opt,i)=>{const display=nm&&nm[opt]?nm[opt].split(" ")[0]:labels?labels[i]:String(opt).length>14?String(opt).slice(0,13)+"…":opt;return <Pill key={opt} label={display} active={active.includes(opt)} color={color} onClick={()=>toggle(set,opt)}/>;})}</div></div>
    ))}
    {hasFilters&&<button onClick={clearAll} style={{width:"100%",background:B.redBg,border:`1px solid ${B.red}44`,color:B.red,borderRadius:6,padding:"6px",fontSize:10,cursor:"pointer",fontFamily:"inherit",fontWeight:700}}>✕ Limpiar filtros</button>}
    <div style={{borderTop:`1px solid ${B.border}`,paddingTop:12,display:"flex",flexDirection:"column",gap:6}}>
      ...
      {[{l:"Ingresos",v:f$(kpi.ing),c:B.red},{l:"Clientes",v:fN(kpi.clis),c:B.white},{l:"Líneas",v:fN(kpi.lineas),c:B.g2}].map(x=>(<div key={x.l} style={{background:B.panel,borderRadius:5,padding:"6px 9px",border:`1px solid ${B.border}`}}><div style={{fontSize:9,color:B.g3}}>{x.l}</div><div style={{color:x.c,fontWeight:800,fontSize:12,marginTop:1}}>{x.v}</div></div>))}
    </div>
  </div>);
  ...
  const ViewResumen=()=>(<div style={{display:"flex",flexDirection:"column",gap:14}}>
    <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
      ...
      <KCard icon="📅" label="Período" value={fyList.slice(-1)[0]||"—"} sub={fyList.join(" · ")||"Carga un Excel"} color={B.blue}/>
    </div>
    {analysis.alerts.filter(a=>a.icon==="🔴").map((a,i)=>(<div key={i} style={{background:B.redBg,border:`1px solid ${B.red}`,borderRadius:8,padding:"10px 14px",display:"flex",gap:10,alignItems:"flex-start"}}><span style={{fontSize:18,flexShrink:0}}>{a.icon}</span><div><div style={{color:B.red,fontWeight:800,fontSize:12}}>{a.t}</div><div style={{color:B.g1,fontSize:11,marginTop:2,lineHeight:1.5}}>{a.b}</div></div></div>))}
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(320px,1fr))",gap:14}}>
      ...
      <Pnl><ST sub="Click para filtrar">Segmentación Pareto</ST>
        {[{s:"TOP",c:B.amberLt},{s:"MED",c:B.red},{s:"LOW",c:B.g3}].map(({s,c})=>{const sc=allClients.filter(x=>x.seg===s);const pct=sc.reduce((sum,x)=>sum+(x.pct||0),0);return(<div key={s} style={{marginBottom:10,cursor:"pointer"}} onClick={()=>toggle(setFSeg,s)}><div style={{display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:3}}><span style={{color:c,fontWeight:700}}>{s} · {sc.length} clientes</span><span style={{color:B.white}}>{pct.toFixed(1)}%</span></div><div style={{height:7,background:B.border,borderRadius:4,overflow:"hidden"}}><div style={{height:"100%",width:`${clamp(pct,0,100)}%`,background:c,borderRadius:4,transition:"width .5s"}}/></div><div style={{display:"flex",gap:4,flexWrap:"wrap",marginTop:4}}>{sc.slice(0,3).map(x=>(<span key={x.id} style={{fontSize:9,color:B.g3,background:B.panel,borderRadius:3,padding:"1px 5px",border:`1px solid ${B.border}`}}>{x.nm.split(" ").slice(0,2).join(" ")}</span>))}</div></div>);})}
      </Pnl>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(320px,1fr))",gap:14}}>
      <Pnl><ST sub="% del ingreso por industria">Mix sectorial</ST>
        <ResponsiveContainer width="100%" height={200}><PieChart>
          <Pie data={allSector.filter(s=>s.ing>0)} dataKey="ing" nameKey="sec" cx="50%" cy="50%" outerRadius={80} innerRadius={38} label={({percent})=>`${(percent*100).toFixed(0)}%`} labelLine={{stroke:B.border,strokeWidth:1}}>
            {allSector.filter(s=>s.ing>0).map((_,i)=><Cell key={i} fill={B.ch[i%B.ch.length]}/>)}
      ...
      <Pnl><ST sub="Ingresos y margen por familia">Rentabilidad por línea</ST>
        <ResponsiveContainer width="100%" height={200}><BarChart data={allFamilia.filter(f=>f.ing>0).slice(0,7)} layout="vertical" margin={{left:0,right:20,top:4,bottom:4}}>
          <CartesianGrid strokeDasharray="3 3" stroke={B.border} horizontal={false}/><XAxis type="number" stroke={B.g3} tick={{fontSize:9}} tickFormatter={f$} width={60}/><YAxis type="category" dataKey="f2" stroke={B.g3} tick={{fontSize:9}} width={140}/><Tooltip content={<TT/>}/><Legend wrapperStyle={{fontSize:10}}/>
  ...
  const ViewClientes=()=>(<div style={{display:"grid",gridTemplateColumns:selCli?"1fr 340px":"1fr",gap:14,alignItems:"start"}}>
    <Pnl style={{overflow:"auto"}}><ST sub={`${filteredClients.length} clientes`}>Cartera de clientes</ST>
      ...
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,tableLayout:"fixed"}}>
        <colgroup><col style={{width:26}}/><col style={{width:"25%"}}/><col style={{width:"13%"}}/><col style={{width:78}}/><col style={{width:74}}/><col style={{width:56}}/><col style={{width:34}}/><col style={{width:40}}/><col/></colgroup>
    ...
    {selCli&&(<div style={{display:"flex",flexDirection:"column",gap:10}}>
      <Pnl style={{borderTop:`2px solid ${selCli.seg==="TOP"?B.amberLt:selCli.seg==="MED"?B.blueLt:B.g3}`}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:7}}>
          {[{l:"Ingresos",v:f$(selCli.ing),c:B.red},{l:"Margen",v:f$(selCli.mgn),c:selCli.mgn>=0?B.greenLt:B.red},{l:"Margen %",v:fP(selCli.mgnP),c:selCli.mgnP<0?B.red:selCli.mgnP>25?B.green:B.amber},{l:"% del total",v:`${selCli.pct?.toFixed(1)||"?"}%`,c:B.amberLt},{l:"Líneas",v:selCli.lineas},{l:"Meses activo",v:selCli.meses||"—"}].map(({l,v,c})=>(<div key={l} style={{background:B.panel,borderRadius:6,padding:"8px 10px",border:`1px solid ${B.border}`}}><div style={{fontSize:9,color:B.g3,marginBottom:2}}>{l}</div><div style={{fontSize:13,fontWeight:800,color:c||B.white}}>{v}</div></div>))}
        </div>
      ...
      <Pnl><ST sub="Asignar ejecutivo">Cartera</ST><div style={{display:"flex",flexWrap:"wrap",gap:5}}>
  ...
  const ViewProductos=()=>(<div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <Pnl><ST sub="Ingresos vs Margen">Comparativa rentabilidad</ST><ResponsiveContainer width="100%" height={200}><BarChart data={allFamilia.filter(f=>f.ing>0)} margin={{top:5,right:20,left:0,bottom:52}}><CartesianGrid strokeDasharray="3 3" stroke={B.border}/><XAxis dataKey="f2" stroke={B.g3} tick={{fontSize:9,angle:-20,textAnchor:"end"}} interval={0} height={64}/><YAxis stroke={B.g3} tick={{fontSize:9}} tickFormatter={f$} width={60}/><Tooltip content={<TT/>}/><Legend wrapperStyle={{fontSize:10}}/><Bar dataKey="ing" name="Ingresos" fill={B.red} radius={[4,4,0,0]}/><Bar dataKey="mgn" name="Margen" radius={[4,4,0,0]}>{allFamilia.filter(f=>f.ing>0).map((f,i)=><Cell key={i} fill={f.mgnP<0?B.g4:B.greenLt}/>)}</Bar></BarChart></ResponsiveContainer></Pnl>
  </div>);
  ...
  const ViewTendencias=()=>(<div style={{display:"flex",flexDirection:"column",gap:14}}>
    {fyList.length>1?(<>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(320px,1fr))",gap:14}}>
        <Pnl><ST sub="Q1=Abr-Jun · Q2=Jul-Sep · Q3=Oct-Dic · Q4=Ene-Mar">Trimestral</ST><ResponsiveContainer width="100%" height={200}><BarChart data={quarterData} margin={{top:5,right:20,left:0,bottom:5}}><CartesianGrid strokeDasharray="3 3" stroke={B.border}/><XAxis dataKey="q" stroke={B.g3} tick={{fontSize:10}}/><YAxis stroke={B.g3} tick={{fontSize:9}} tickFormatter={f$} width={62}/><Tooltip content={<TT/>}/><Legend wrapperStyle={{fontSize:10}}/>{fyList.map((fy,i)=><Bar key={fy} dataKey={fy} fill={B.ch[i%B.ch.length]} radius={[4,4,0,0]}/>)}</BarChart></ResponsiveContainer></Pnl>
        <Pnl><ST sub="YoY">Totales anuales</ST>{fyTotals.map((fy,i)=>{const prev=fyTotals[i-1];const d=prev?(fy.ing-prev.ing)/prev.ing*100:null;return(<div key={fy.fy} style={{marginBottom:10,background:B.panel,borderRadius:8,padding:"11px 14px",border:`1px solid ${B.border}`,borderLeft:`3px solid ${B.ch[i%B.ch.length]}`}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><span style={{color:B.ch[i%B.ch.length],fontWeight:800,fontSize:12}}>{fy.fy}</span>{d!=null&&<span style={{color:d>=0?B.greenLt:B.red,fontWeight:700,fontSize:11}}>{d>=0?"▲":"▼"}{Math.abs(d).toFixed(1)}% YoY</span>}</div><div style={{display:"flex",gap:20,marginTop:6}}><div><div style={{fontSize:9,color:B.g3}}>INGRESOS</div><div style={{fontSize:14,fontWeight:800,color:B.red}}>{f$(fy.ing)}</div></div><div><div style={{fontSize:9,color:B.g3}}>MARGEN</div><div style={{fontSize:14,fontWeight:800,color:fy.mgnP>15?B.greenLt:B.amberLt}}>{fP(fy.mgnP)}</div></div></div></div>);})}</Pnl>
      </div>
    </>):(<Pnl style={{textAlign:"center",padding:"48px 32px"}}><div style={{fontSize:36,marginBottom:12}}>📊</div><div style={{color:B.white,fontWeight:700,fontSize:14,marginBottom:8}}>Carga más Excels para ver comparativas</div><div style={{color:B.g3,fontSize:12}}>Agrega archivos de otros meses o años fiscales.</div></Pnl>)}
  </div>);

  const ViewEjecutivos=()=>(<div style={{display:"flex",flexDirection:"column",gap:14}}>
    <Pnl><ST sub="Asigna nombres reales — guardado localmente">Configurar ejecutivos</ST><div style={{display:"flex",flexWrap:"wrap",gap:10}}>{opts.reps.map(rep=>(<div key={rep} style={{display:"flex",alignItems:"center",gap:8,background:B.panel,borderRadius:7,padding:"7px 12px",border:`1px solid ${B.border}`}}><span style={{color:B.g3,fontSize:10,minWidth:52,flexShrink:0}}>Rep {rep}</span><span style={{color:B.border}}>→</span><input value={repNames[rep]||""} placeholder="Nombre…" onChange={e=>setRepNames(p=>({...p,[rep]:e.target.value}))} style={{background:"transparent",border:"none",borderBottom:`1px solid ${B.border}`,color:B.amberLt,fontFamily:"inherit",fontSize:11,padding:"2px 4px",outline:"none",width:155}}/></div>))}</div></Pnl>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(320px,1fr))",gap:14}}>
      <Pnl><ST sub="Click para filtrar">Ingresos por ejecutivo</ST><ResponsiveContainer width="100%" height={220}><BarChart data={allReps.map(r=>({...r,nm:repNames[r.rep]||`Rep ${r.rep}`}))} margin={{top:5,right:20,left:0,bottom:52}} onClick={d=>d?.activePayload&&toggle(setFRep,d.activePayload[0]?.payload?.rep)}><CartesianGrid strokeDasharray="3 3" stroke={B.border}/><XAxis dataKey="nm" stroke={B.g3} tick={{fontSize:9,angle:-15,textAnchor:"end"}} interval={0} height={64}/><YAxis stroke={B.g3} tick={{fontSize:9}} tickFormatter={f$} width={60}/><Tooltip content={<TT/>}/><Bar dataKey="ing" name="Ingresos" radius={[4,4,0,0]} cursor="pointer">{allReps.map((_,i)=><Cell key={i} fill={B.ch[i%B.ch.length]}/>)}</Bar></BarChart></ResponsiveContainer></Pnl>
      <div style={{display:"flex",flexDirection:"column",gap:8}}>{allReps.map((r,i)=>(<div key={r.rep} style={{background:B.panel,borderRadius:9,padding:"11px 14px",border:`1px solid ${B.border}`,borderLeft:`3px solid ${B.ch[i%B.ch.length]}`,cursor:"pointer"}} onClick={()=>toggle(setFRep,r.rep)}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}><div style={{fontWeight:800,color:B.ch[i%B.ch.length],fontSize:12,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{repNames[r.rep]||`Rep ${r.rep}`}</div><div style={{color:B.red,fontWeight:700,fontSize:12,flexShrink:0,marginLeft:8}}>{f$(r.ing)}</div></div><div style={{display:"flex",gap:14,fontSize:10,color:B.g3}}><span>{r.clientes} clientes</span><span>{r.lineas} líneas</span><span>{kpi.ing?(r.ing/kpi.ing*100).toFixed(1):0}%</span></div><div style={{marginTop:6,height:3,background:B.border,borderRadius:2}}><div style={{height:"100%",width:`${allReps[0]?.ing?clamp(r.ing/allReps[0].ing*100,0,100):0}%`,background:B.ch[i%B.ch.length],borderRadius:2}}/></div></div>))}</div>
    </div>
  </div>);
  ...
  const ViewPivot=()=>{const{rows:pr,cols:pc}=pivotData;const vFmt=PVALS[pVal]?.f||fN;const gt=pr.reduce((s,r)=>s+r.total,0);return(<div style={{display:"flex",flexDirection:"column",gap:14}}>
    <Pnl><ST sub="Se actualiza al instante">Constructor tabla dinámica</ST><div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
  ...
  const ViewAnalisis=()=>{const{alerts,opps,actions}=analysis;return(<div style={{display:"flex",flexDirection:"column",gap:14}}>
    <div style={{background:`linear-gradient(135deg,${B.redBg} 0%,${B.panel} 100%)`,border:`1px solid ${B.red}44`,borderRadius:10,padding:"16px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:12}}><div><div style={{fontWeight:900,fontSize:15,color:B.white}}>ANÁLISIS EJECUTIVO AUTOMÁTICO</div><div style={{fontSize:10,color:B.g3,marginTop:3}}>Motor integrado · sin API externa · se actualiza con filtros</div></div><div style={{textAlign:"right"}}><div style={{fontSize:22,fontWeight:900,color:B.red}}>{f$(kpi.ing)}</div><div style={{fontSize:11,color:kpi.mgnP>20?B.greenLt:kpi.mgnP>10?B.amberLt:B.red,fontWeight:700}}>{fP(kpi.mgnP)} margen</div></div></div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(320px,1fr))",gap:14}}>
      <Pnl style={{borderLeft:`3px solid ${B.red}`}}><ST>🔴 Alertas críticas</ST>{alerts.length===0?<div style={{color:B.greenLt,fontSize:11}}>✅ Sin alertas</div>:alerts.map((a,i)=>(<div key={i} style={{marginBottom:12,paddingLeft:9,borderLeft:`2px solid ${B.red}44`}}><div style={{display:"flex",gap:6,alignItems:"flex-start",marginBottom:3}}><span style={{fontSize:13,flexShrink:0}}>{a.icon}</span><span style={{fontWeight:700,fontSize:11,color:B.white}}>{a.t}</span></div><div style={{fontSize:10,color:B.g1,lineHeight:1.6,paddingLeft:19}}>{a.b}</div></div>))}</Pnl>
      <Pnl style={{borderLeft:`3px solid ${B.green}`}}><ST>🟢 Oportunidades</ST>{opps.length===0?<div style={{color:B.g3,fontSize:11}}>Sin oportunidades detectadas</div>:opps.map((o,i)=>(<div key={i} style={{marginBottom:12,paddingLeft:9,borderLeft:`2px solid ${B.green}44`}}><div style={{display:"flex",gap:6,alignItems:"flex-start",marginBottom:3}}><span style={{fontSize:13,flexShrink:0}}>{o.icon}</span><span style={{fontWeight:700,fontSize:11,color:B.white}}>{o.t}</span></div><div style={{fontSize:10,color:B.g1,lineHeight:1.6,paddingLeft:19}}>{o.b}</div></div>))}</Pnl>
    </div>
    <Pnl style={{borderLeft:`3px solid ${B.amberLt}`}}><ST>🎯 Prioridades esta semana</ST><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:10}}>{actions.map((a,i)=>(<div key={i} style={{background:B.panel,borderRadius:8,padding:"11px 13px",border:`1px solid ${B.border}`,display:"flex",gap:10,alignItems:"flex-start"}}><div style={{fontSize:14,fontWeight:900,color:B.white,background:B.red,borderRadius:4,width:24,height:24,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,lineHeight:1}}>{i+1}</div><div style={{fontSize:11,color:B.white,lineHeight:1.6}}>{a}</div></div>))}</div></Pnl>
    <Pnl><ST sub="Eje X=Ingresos · Eje Y=Margen%">Mapa de rentabilidad</ST><ResponsiveContainer width="100%" height={260}><ScatterChart margin={{top:10,right:20,left:0,bottom:10}}><CartesianGrid strokeDasharray="3 3" stroke={B.border}/><XAxis type="number" dataKey="ing" name="Ingresos" stroke={B.g3} tick={{fontSize:9}} tickFormatter={f$} width={60}/><YAxis type="number" dataKey="mgnP" name="Margen%" stroke={B.g3} tick={{fontSize:9}} tickFormatter={v=>`${v.toFixed(0)}%`} width={40}/><ZAxis type="number" dataKey="lineas" range={[40,450]}/><Tooltip cursor={{strokeDasharray:"3 3"}} content={({active,payload})=>{if(!active||!payload?.length)return null;const d=payload[0]?.payload;return<div style={{background:B.panel,border:`1px solid ${B.border}`,borderRadius:8,padding:"9px 13px",fontSize:11}}><p style={{color:B.red,fontWeight:700,marginBottom:4}}>{d?.nm}</p><p style={{color:B.white}}>Ingresos: {f$(d?.ing)}</p><p style={{color:d?.mgnP<0?B.red:B.greenLt}}>Margen: {fP(d?.mgnP)}</p><p style={{color:B.g3}}>{d?.lineas} líneas · {d?.sector}</p></div>;}}/><Scatter data={allClients.filter(c=>c.ing>0)} fill={B.red}>{allClients.filter(c=>c.ing>0).map((c,i)=><Cell key={i} fill={c.mgnP<0?B.red:c.mgnP>30?B.greenLt:B.amberLt}/>)}</Scatter></ScatterChart></ResponsiveContainer></Pnl>
  </div>);};

  const ViewFirebase=()=>(<div style={{display:"flex",flexDirection:"column",gap:14}}>
    <Pnl style={{borderLeft:`3px solid ${B.amber}`}}><ST>📋 Activar modo compartido con Firebase</ST><div style={{display:"flex",flexDirection:"column",gap:12}}>{[{n:1,t:"Crear proyecto",d:"console.firebase.google.com → Crear proyecto → cummins-zona-sur"},{n:2,t:"Activar Storage",d:"Menú → Storage → Comenzar → modo prueba"},{n:3,t:"Obtener credenciales",d:"⚙️ → General → Tus apps → </> → Registrar → copiar firebaseConfig"},{n:4,t:"Agregar a código",d:"Edita src/App.jsx con tu config y redespliega en GitHub"}].map(s=>(<div key={s.n} style={{display:"flex",gap:12}}><div style={{width:26,height:26,borderRadius:5,background:B.red,color:B.white,fontWeight:900,fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{s.n}</div><div><div style={{fontWeight:700,color:B.white,fontSize:12,marginBottom:2}}>{s.t}</div><div style={{fontSize:11,color:B.g2,lineHeight:1.5}}>{s.d}</div></div></div>))}</div></Pnl>
    <Pnl><ST sub="Archivos cargados esta sesión">Datos en memoria</ST>{fileAggs.length===0?<div style={{color:B.g3,fontSize:11}}>Ningún archivo — usa + EXCELS</div>:(<><table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}><thead><tr style={{borderBottom:`1px solid ${B.border}`}}>{["Archivo","Suc.","FY","Ventas","Clientes","Ingresos"].map((h,i)=>(<th key={h} style={{padding:"5px 10px",textAlign:i===5?"right":"left",color:B.g3,fontSize:9,letterSpacing:".08em",textTransform:"uppercase"}}>{h}</th>))}</tr></thead><tbody>{fileAggs.map((f,i)=>(<tr key={i} style={{borderBottom:`1px solid ${B.border}22`}}><td style={{padding:"7px 10px",color:B.white,fontWeight:600}}>{f.label}</td><td style={{padding:"7px 10px"}}><span style={{background:B.redBg,color:B.red,borderRadius:3,padding:"2px 7px",fontSize:10,fontWeight:700}}>{f.sucursal}</span></td><td style={{padding:"7px 10px",color:B.g3}}>{f.fyl} {f.fml}</td><td style={{padding:"7px 10px",color:B.g3}}>{f.salesRows}</td><td style={{padding:"7px 10px",color:B.g3}}>{f.clients.length}</td><td style={{padding:"7px 10px",fontWeight:700,color:B.red,textAlign:"right"}}>{f$(f.totalIng)}</td></tr>))}<tr style={{borderTop:`1px solid ${B.border}`,background:B.panel}}><td style={{padding:"7px 10px",fontWeight:800,color:B.red}} colSpan={3}>TOTAL</td><td style={{padding:"7px 10px",color:B.g3}}>{fN(fileAggs.reduce((s,f)=>s+f.salesRows,0))}</td><td style={{padding:"7px 10px",color:B.g3}}>{fN(new Set(fileAggs.flatMap(f=>f.clients.map(c=>c.id))).size)}</td><td style={{padding:"7px 10px",fontWeight:900,color:B.red,textAlign:"right"}}>{f$(fileAggs.reduce((s,f)=>s+f.totalIng,0))}</td></tr></tbody></table>
    <div style={{marginTop:12,display:"flex",flexDirection:"column",gap:8}}>{fileAggs.slice(0,3).map((f,i)=>(<div key={i} style={{background:B.panel,borderRadius:7,padding:"9px 12px",border:`1px solid ${B.border}`}}><div style={{fontWeight:700,color:B.white,fontSize:10,marginBottom:6}}>{f.label} — columnas detectadas</div><div style={{display:"flex",flexWrap:"wrap",gap:4}}>{Object.entries(f.cols).map(([k,v])=>(<span key={k} style={{fontSize:9,padding:"1px 6px",borderRadius:3,background:v?B.greenDim:B.redDim,border:`1px solid ${v?B.green:B.red}`,color:v?B.greenLt:B.red}}>{k}:{v?`"${v}"`:"❌"}</span>))}</div></div>))}</div></>)}</Pnl>
  </div>);

  const renderView=()=>{switch(view){case "RESUMEN":return<ViewResumen/>;case "CLIENTES":return<ViewClientes/>;case "PRODUCTOS":return<ViewProductos/>;case "SECTORES":return<ViewSectores/>;case "TENDENCIAS":return<ViewTendencias/>;case "EJECUTIVOS":return<ViewEjecutivos/>;case "TABLA DIN.":return<ViewPivot/>;case "ANÁLISIS":return<ViewAnalisis/>;case "FIREBASE":return<ViewFirebase/>;default:return<ViewResumen/>;}}
  ...
  return(<div style={{minHeight:"100vh",background:B.bg,color:B.white,fontFamily:"'Helvetica Neue',Helvetica,Arial,sans-serif",display:"flex",flexDirection:"column"}}>
    ...
    <div style={{background:B.red,padding:"0 16px",display:"flex",alignItems:"center",height:54,flexShrink:0,boxShadow:"0 2px 12px rgba(120,0,0,.5)"}}>
      <div style={{display:"flex",alignItems:"center",gap:10,paddingRight:16,borderRight:"1px solid rgba(255,255,255,.25)",marginRight:14,flexShrink:0}}><Logo/><div style={{lineHeight:1.2}}><div style={{fontWeight:900,fontSize:13,color:B.white,letterSpacing:".06em"}}>CUMMINS PERÚ</div><div style={{fontSize:8,color:"rgba(255,255,255,.6)",letterSpacing:".2em",marginTop:1}}>ZONA SUR · COMERCIAL</div></div></div>
      <nav style={{display:"flex",flex:1,overflowX:"auto",gap:0,msOverflowStyle:"none",scrollbarWidth:"none"}}>{VIEWS.map(v=>(<button key={v} onClick={()=>setView(v)} style={{background:view===v?"rgba(0,0,0,.28)":"transparent",border:"none",borderBottom:view===v?"2px solid #fff":"2px solid transparent",color:view===v?B.white:"rgba(255,255,255,.58)",fontFamily:"inherit",fontSize:10,fontWeight:view===v?800:500,letterSpacing:".06em",padding:"15px 12px 13px",cursor:"pointer",transition:"all .12s",whiteSpace:"nowrap",flexShrink:0}}>{v}</button>))}</nav>
      <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",paddingLeft:14,borderLeft:"1px solid rgba(255,255,255,.25)",marginLeft:10,flexShrink:0}}><span style={{fontWeight:900,color:B.white,fontSize:13,lineHeight:1}}>{f$(kpi.ing)}</span><span style={{color:kpi.mgnP>20?"#86EFAC":kpi.mgnP>10?"#FDE68A":"#FCA5A5",fontWeight:700,fontSize:9,marginTop:2}}>{fP(kpi.mgnP)} margen</span></div>
      <button onClick={()=>document.getElementById("fi").click()} style={{background:"rgba(0,0,0,.3)",border:"1px solid rgba(255,255,255,.38)",color:B.white,borderRadius:5,padding:"6px 13px",fontSize:10,cursor:"pointer",fontFamily:"inherit",fontWeight:800,letterSpacing:".06em",flexShrink:0,marginLeft:10,whiteSpace:"nowrap"}}>{loading?"⚙ Cargando…":"＋ EXCELS"}</button>
      <input id="fi" type="file" accept=".xlsx,.xls" multiple style={{display:"none"}} onChange={e=>ingest(Array.from(e.target.files))}/>
    </div>
    {fileAggs.length>0&&(<div style={{background:B.surf,borderBottom:`1px solid ${B.border}`,padding:"4px 16px",display:"flex",gap:6,alignItems:"center",flexShrink:0,overflowX:"auto",height:30,msOverflowStyle:"none",scrollbarWidth:"none"}}><span style={{fontSize:9,color:B.g3,letterSpacing:".12em",fontWeight:700,flexShrink:0}}>DATOS:</span>{fileAggs.map((f,i)=>(<span key={i} style={{fontSize:9,background:B.card,border:`1px solid ${B.border}`,borderRadius:3,padding:"1px 7px",color:B.g3,borderLeft:`2px solid ${B.red}`,whiteSpace:"nowrap",flexShrink:0}}>{f.label} · {f$(f.totalIng)}</span>))}<span style={{fontSize:9,color:B.g3,marginLeft:4,flexShrink:0}}>Total: {f$(fileAggs.reduce((s,f)=>s+f.totalIng,0))}</span></div>)}
    {!hasData?(<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",flex:1,gap:28,padding:32}}>
      <div style={{textAlign:"center"}}><div style={{fontWeight:900,fontSize:32,color:B.red,letterSpacing:"-.01em",lineHeight:1}}>INTELIGENCIA COMERCIAL</div><div style={{color:B.g3,fontSize:13,marginTop:8,letterSpacing:".15em"}}>CUMMINS PERÚ · ZONA SUR</div></div>
      <div onDrop={e=>{e.preventDefault();ingest(Array.from(e.dataTransfer.files));}} onDragOver={e=>e.preventDefault()} onClick={()=>document.getElementById("fi").click()} style={{border:`2px dashed ${dragging?B.red:B.border}`,borderRadius:16,padding:"48px 64px",textAlign:"center",cursor:"pointer",transition:"all .2s",maxWidth:480}}>
        <div style={{fontSize:44,marginBottom:14}}>📂</div><div style={{fontSize:16,fontWeight:700,color:B.white,marginBottom:8}}>Arrastra tus Excels aquí</div>
        <div style={{color:B.g3,fontSize:12,lineHeight:1.7}}>Carga uno o <strong style={{color:B.red}}>cientos de archivos</strong> a la vez<br/>Pre-agrega al vuelo — sin lag con 100+ Excels<br/><code style={{color:B.red,fontSize:11}}>2026_04_K27.xlsx · 2025_11_K23.xlsx</code></div>
      </div>
      <div style={{display:"flex",gap:20,color:B.g3,fontSize:11,flexWrap:"wrap",justifyContent:"center"}}>{["Pre-agregación instantánea","100+ Excels sin lag","Año fiscal Abr→Mar","Filtros cruzados","Tabla dinámica","Análisis automático"].map(f=>(<span key={f} style={{display:"flex",alignItems:"center",gap:5}}><span style={{color:B.red}}>✓</span>{f}</span>))}</div>
    </div>):(
      <div style={{display:"flex",flex:1,overflow:"hidden",minHeight:0}}>
        <div style={{width:206,flexShrink:0,background:B.surf,borderRight:`1px solid ${B.border}`,padding:14,overflowY:"auto",minHeight:0}}><Sidebar/></div>
        <div className="fade" key={view} style={{flex:1,padding:"18px 20px",overflowY:"auto",minHeight:0,minWidth:0}}>{renderView()}</div>
      </div>
    )}
